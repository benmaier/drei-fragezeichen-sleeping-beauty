...
for album in albums:
    ...
    popgauge = Counter(pop[:12]).most_common()[0][0]
    pop -= popgauge
    t = np.array(t)
...
for i, obs in enumerate(observations):
    ...
    if len(obs)>6:
        ...
        probability_lower_than_minus_one[i] = np.count_nonzero(np.array(obs)< -1)/len(obs)

mbins = 0.5*(bins[1:]+bins[:-1])
mean_diff[1] = np.nan
std_diff[1] = np.nan
probability_lower_than_zero[1] = np.nan
probability_lower_than_minus_one[1] = np.nan

ax[1].plot(mbins,probability_lower_than_zero)
ax[1].plot(mbins,probability_lower_than_minus_one)
